...
use constant DEFAULT_RIGHTMARGIN => 76; # text.c breite=76

{
  ...
  sub _have_multibyte {
    $help_done ||= do {
      ...
      $have_multibyte = (defined $help && $help =~ /\s-u[, ]/);
      ### $have_multibyte
      1;
    };
    return $have_multibyte;
  }
}
...
=for stopwords HTML-FormatExternal vilistextum sourceforge.net formatters charset Ryde
...
There's no C<input_charset> option yet.  (The C<-y> to C<vilistextum> might
be only a default, with the document C<< <meta> >> taking precedence,
whereas the intention of C<input_charset> is to override the document.)
